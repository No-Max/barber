...
if ($_POST) { 
  ...
  $subject = $name." оставил заявку на создание сайта";
  ...
  $emailgo->to_email   = 'gratimaleschenko@gmail.com';  // Куда будет отправлено письмо 
  $emailgo->to_name    = 'Админ'; // Имя получателя
  $emailgo->subject    = $subject; // Тема
  $emailgo->body      = $phone; // Сообщение
  $emailgo->send(); 
  $json['error'] = 0;
  echo json_encode($json); 
} else { 
